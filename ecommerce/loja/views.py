...
from .models import *
import uuid
from .utilidades import filtrar_produtos, preco_minimo_maximo, ordenar_produtos, enviar_email, exportar_csv
...
def finalizar_pedido(request, id_pedido):
    if request.method == "POST":
        ...
        if erro:
            endereco = Endereco.objects.filter(cliente=pedido.cliente)
            context = {'erro': erro, 'pedido': pedido, 'endereco':endereco}
            return render(request, 'checkout.html', context)
        else:
            itens_pedido = ItemPedido.objects.filter(pedido=pedido)
            # pega o link completo do nosso site
        ...
        if 'email' in dados and 'senha' in dados and 'confirmacao_senha' in dados:
            ...
            if senha == confirmacao_senha:
                ...
                if not criado:
                    ...
                    login(request, usuario)
                    # criar  cliente
                    # verificar se existe id sessao nos cookies do navegador
                    if request.COOKIES.get('id_sessao'):
                        id_sessao = request.COOKIES.get('id_sessao')
                        cliente, criado = Cliente.objects.get_or_create(id_sessao=id_sessao)
                    else:
...
def gerenciar_loja(request):
    if request.user.groups.filter(name='equipe').exists():
        ...
        qtde_pedidos = len(pedidos)
        faturamento = sum([pedido.preco_total for pedido in pedidos])
        qtde_produtos = sum([pedido.quantidade_total for pedido in pedidos])
